...
export default function QuestionsPage() {
  ...
  const filteredQuestions = questions.filter(q => {
    ...
    if (filter.difficulty && q.difficulty !== Number(filter.difficulty)) return false;
  ...
  const stats = questions.reduce((acc, q) => {
    if (q.subject === 'Физика') {
      const key = `${q.subject}-level${q.difficulty || 0}`;
      acc[key] = (acc[key] || 0) + 1;
    } else {
            ...
            {SUBJECTS.map(subject => {
              if (subject === 'Физика') {
                const levelCounts = Array.from(
                  { length: PHYSICS_LEVELS },
                  (_, i) => stats[`${subject}-level${i + 1}`] || 0
                );
                return (
